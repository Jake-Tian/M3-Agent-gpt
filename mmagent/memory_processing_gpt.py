...
from .utils.chat_api import (
    ...
    generate_messages as generate_messages_api,
    get_response_with_retry,
)
from .utils.token_monitor import add_tokens
from .utils.video_processing import get_video_base64_without_audio
...
def generate_all_memories(video_context, model_type="sft", video_id=None):
    input = [
        {
            "type": "text",
    ...
    messages = generate_messages_api(input)
    epi_key = "video_descriptions"
    sem_key = "high_level_conclusions"

    memories = None
    for i in range(MAX_RETRIES):
        response = get_response_with_retry(GPT_MODEL, messages, timeout=60)
        if response and len(response) >= 2:
            add_tokens("memory", response[1], subkey="generation", video_id=video_id)
        memories_string = response[0] if response else None
...
def generate_memories(
    base64_frames, faces_list, voices_list, clip_path, model_type="sft", video_id=None
):
    # GPT cannot process audio; strip audio from video and use transcriptions (voices_list) for speech
    base64_video = get_video_base64_without_audio(clip_path)
    video_context = generate_video_context(
        base64_frames, faces_list, voices_list, base64_video
    )
    episodic_memories, semantic_memories = generate_all_memories(
        video_context, model_type, video_id
    )
    return episodic_memories, semantic_memories


def process_memories(video_graph, memory_contents, clip_id, type="episodic", video_id=None):
    def get_memory_embeddings(memory_contents):
        # calculate the embedding for each memory
        model = "text-embedding-3-large"
        result = parallel_get_embedding(model, memory_contents)
        embeddings = result[0]
        if len(result) >= 2:
            add_tokens("memory", result[1], subkey="embedding", video_id=video_id)
        return embeddings
